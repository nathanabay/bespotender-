...
def run():
    print("--- 📊 INJECTING DUMMY DATA (SQL OVERRIDE MODE) ---")
    ...
    for s in scenarios:
        if not frappe.db.exists("Tender Opportunity", {"title": s["title"]}):
            # 1. INSERT AS DRAFT (Safe Mode)
            doc = frappe.get_doc({
                "doctype": "Tender Opportunity",
                "title": s["title"],
                "sector": s["sector"],
                "tender_number": f"TEST-{random.randint(10000, 99999)}",
            ...
            # 2. SQL FORCE UPDATE (The Bypass)
            # We directly manipulate the database row, ignoring all workflow rules
            frappe.db.sql("""
                UPDATE `tabTender Opportunity`
                SET workflow_state = %s, status = %s
                WHERE name = %s
            """, (s["state"], "Closed" if s["state"] in ["Won", "Lost"] else "Open", doc.name))
            
            print(f"✔ Created & Forced: {s['title']} -> {s['state']}")
            count += 1
        else:
