...
def run():
    print("--- 📐 FINALIZING TENDER LAYOUT (SANITIZED) ---")

    dt = "Tender Opportunity"
    
    # 1. SANITIZE DATA (Fix Truncation Error)
    # Reset bad probability scores to 0 so we can modify the column safely
    # We check for values that might be out of range for a standard decimal(21,9) or illogical percentages
    frappe.db.sql("UPDATE `tabTender Opportunity` SET bid_probability_score = 0 WHERE bid_probability_score > 100 OR bid_probability_score < 0")
    print("✔ Sanitized Data in 'bid_probability_score'")

    doc = frappe.get_doc("DocType", dt)
    
    # 2. HARVEST EXISTING FIELD OBJECTS
    # We map them by fieldname so we can reuse the specific object connected to the DB column
    existing_fields = {f.fieldname: f for f in doc.fields}

    # 3. DEFINE THE MASTER STRUCTURE
    # Format: (Fieldtype, Label, Fieldname, Options/Description)
    structure = [
        ...
        ("Currency", "Document Price", "document_price", None), # <--- MOVED HERE
    ...
    for idx, (ftype, label, fname, options) in enumerate(structure):
        if fname in added_names:
            continue # Skip duplicates strictly

        if fname in existing_fields:
            # REUSE EXISTING FIELD (Keeps data safe)
            f = existing_fields[fname]
            f.label = label
            f.fieldtype = ftype
            f.idx = idx + 1 # Force strict order
