...
def run():
    print("--- 🏗️ SAFE WORKSPACE REBUILD ---")
    ...
    frappe.get_doc({
        ...
        "chart_type": "Count",  # Simplest type
    ...
    # We turned these off in the Rescue step. Turning them back on.
    frappe.db.sql("UPDATE `tabClient Script` SET enabled=1 WHERE module='Tender Management'")
    print("✔ Re-enabled Client Logic (Auto-IDs, Validation)")
