...
def run():
    print("--- ☢️ RUNNING NUCLEAR WORKFLOW FIX ---")

    # 1. STATES
    # We know States worked in previous attempts, so we use the same logic
    states = [
    ...
    for s_name, s_style in states:
        if not frappe.db.exists("Workflow State", s_name):
            # Check SQL directly
            exists_sql = frappe.db.sql("select name from `tabWorkflow State` where name=%s", (s_name,))
            if not exists_sql:
                frappe.db.sql("""
    ...
    # 2. ACTIONS (The Fix)
    # We insert ONLY the 'name' column. We ignore 'action'/'label' columns completely.
    ...
    for a in actions:
        if not frappe.db.exists("Workflow Action", a):
            # Check SQL directly
            exists_sql = frappe.db.sql("select name from `tabWorkflow Action` where name=%s", (a,))
            if not exists_sql:
                frappe.db.sql("""
    ...
    frappe.db.commit()
    print("--- ✅ FINAL SUCCESS: TENDER SYSTEM INSTALLED ---")
