...
def run():
    print("--- 🚚 CONFIGURING PURCHASE LOGISTICS & FINANCE WORKFLOW ---")

    dt = "Tender Opportunity"
    doc = frappe.get_doc("DocType", dt)
    
    # 1. ADD LOGISTICS FIELDS
    new_fields = [
        frappe.new_doc("DocField", {
            "fieldname": "tender_purchaser",
            "label": "Tender Purchaser",
            "fieldtype": "Link",
            "options": "User",
            "description": "The employee responsible for buying the document",
            "reqd": 0 
        }),
        frappe.new_doc("DocField", {
            ...
            "fieldtype": "Data",
            "description": "Name of the driver assigned for this trip"
        })
    ]

    # 2. UPDATE STATUS OPTIONS
    # We need specific statuses for this chain of events
    ...
    # 3. INJECT FIELDS (Logistics Section)
    final_fields = []
    inserted = False
    ...
    frappe.db.sql("UPDATE `tabTender Opportunity` SET doc_purchase_status = 'Pending Assignment'")
...
frappe.ui.form.on('Tender Opportunity', {
    refresh: function(frm) {
        ...
        if (frm.doc.doc_purchase_status == 'Pending Assignment' && is_manager) {
