...
def run():
    print("--- 🏗 FINAL SOP RECONSTRUCTION ---")
    ...
    # 2. FORCE WORKFLOW ACTIONS (SQL METHOD)
    ...
    print("... Forcing Actions")
    for a in actions:
        exists = frappe.db.sql("SELECT name FROM `tabWorkflow Action` WHERE name=%s", (a,))
        if not exists:
            # Insert ONLY name to avoid column errors
            frappe.db.sql("INSERT INTO `tabWorkflow Action` (name, creation, modified, modified_by, owner, docstatus) VALUES (%s, NOW(), NOW(), 'Administrator', 'Administrator', 0)", (a,))
            print(f"✔ Action Forced: {a}")
    ...
    sop_states = [
        ("Draft", "Inverse", 0),
        ("Under Evaluation", "Info", 0),
        ("Approved to Bid", "Success", 0),
        ("Tender Purchased", "Primary", 0),
        ("Bid Bond Issued", "Primary", 0),
        ("Technical Preparation", "Warning", 0),
        ("Financial Preparation", "Warning", 0),
        ("Ready for Submission", "Danger", 0),
        ("Submitted", "Primary", 1),
        ("Under Client Evaluation", "Warning", 1),
        ("Won", "Success", 1),
        ("Lost", "Danger", 2)
    ]
    ...
    # Bypass link validation for speed/safety
