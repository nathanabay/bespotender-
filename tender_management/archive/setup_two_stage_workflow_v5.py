...
def run():
    print("--- 👮 SETTING UP 2-STAGE APPROVAL WORKFLOW (LIFECYCLE FIX) ---")
    ...
    # 3. DEFINE STATES (The Fix: Rejected is now doc_status=0)
    states = [
        # State Name, Doc Status (0=Draft, 1=Sub, 2=Canc), Edit Role, Style
        ...
        {"state": "Approved to Bid", "doc_status": 1, "allow_edit": "Tender Director", "style": "Success"}, 
        {"state": "Rejected", "doc_status": 0, "allow_edit": "Tender Manager", "style": "Danger"} # <--- FIXED: doc_status=0
    ]
    ...
    for s in states:
        if not frappe.db.exists("Workflow State", s["state"]):
            ...
            # Update existing state to ensure doc_status is correct
            doc = frappe.get_doc("Workflow State", s["state"])
            doc.style = s["style"]
    ...
    print("--- ✅ WORKFLOW SUCCESSFULLY ACTIVATED ---")
